    ...
    if (check_bitrix_sessid()) {
        if (empty($arParams["REQUIRED_FIELDS"]) || !in_array("NONE", $arParams["REQUIRED_FIELDS"])) {
            ...
            if ((empty($arParams["REQUIRED_FIELDS"]) || in_array("LAST_NAME", $arParams["REQUIRED_FIELDS"])) && strlen($_POST["user_last_name"]) <= 1)
                $arResult["ERROR_MESSAGE"][] = "Поле \"ФИО\" не должно быть пустым";
            ...
            // if ((empty($arParams["REQUIRED_FIELDS"]) || in_array("ACTIVITY", $arParams["REQUIRED_FIELDS"])) && strlen($_POST["user_chek"]) <= 1)
            //     $arResult["ERROR_MESSAGE"][] = "Поле \"Средний чек\" не должно быть пустым";
            // if ((empty($arParams["REQUIRED_FIELDS"]) || in_array("ACTIVITY", $arParams["REQUIRED_FIELDS"])) && strlen($_POST["user_turnover"]) <= 1)
            //     $arResult["ERROR_MESSAGE"][] = "Поле \"Средний чек\" не должно быть пустым";
        }
        ...
        if (empty($arResult["ERROR_MESSAGE"])) {
            $arFields = array(
                ...
                "AUTHOR_CHEK" => $_POST["user_chek"],
                "AUTHOR_TURNOVER" => $_POST["user_turnover"],
                "AUTHOR_EMPLOYEES" => $_POST["user_employees"],
